...
module.exports = (keystone) => {
  ...
  // SIGN UP
  // POST /sign-up
  router.post('/sign-up', keystone.middleware.cors, (req, res, next) => {
    console.log(req)
    ...
    Promise.resolve(req.body.credentials)
      ...
      .then(user => new Model(user).save())
      // send the new user object back with status 201, but `password`
  ...
  // SIGN IN
  // POST /sign-in
  router.post('/sign-in', keystone.middleware.cors, (req, res, next) => {
    const pw = req.body.credentials.password
    let user

    // find a user based on the email that was passed
    User.model.findOne({ email: req.body.credentials.email })
      .then(record => {
  ...
  router.patch('/change-password', keystone.middleware.cors, requireToken, (req, res, next) => {
    let user
    // `req.user` will be determined by decoding the token payload
    User.model.findById(req.user.id)
  ...
  router.delete('/sign-out', keystone.middleware.cors, requireToken, (req, res, next) => {
